...
function OnPlayerDisconnect(playerid, reason)
    ...
    AC[playerid].lasthit = nil;
	AC[playerid].unconscious = nil;
...
function OnPlayerDeath(playerid, p_classid, killerid, k_classid)
    ...
    if (AC[playerid].lasthit ~= killerid) and (AC[playerid].unconscious == nil) and (killerid ~= -1) and (killerid ~= playerid) then 
		SendMessageToAll(255, 0, 0, GetPlayerName(killerid).." u¿y³ cheat'u i zosta³ zbanowany.");
		Ban(killerid);
	end
